const chatUrl = "https://localhost:1000/chat";
...
//После теста раскомментировать при необходимости
function login() {
    ...
    if (userName && password) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                try {
                    ...
                    if (xhr.status == 200) {
                        $.post("/Home/Login", { x: String(_token) });
                    }
                    else {
                        console.log("Status: ", xhr.response.status);
                    }
                } catch (e) {
        ...
        xhr.open("POST", "https://localhost:1000/connect/token", true);
